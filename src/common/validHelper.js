    ...
    for (var j = 0; j < validDomArray.length; j++) {
      ...
      if (validStr) {
        ...
        if (!val) {
          return {
            pass: false,
            msg: '请' + d.getAttribute('placeholder'),
            dom: d
          }
        }
      }
...
function check(tempDom, validObj) {
  let tipWord = ''
  let val = ''
  if (validObj.type.indexOf('select') != -1) {
    //下拉框
    ...
    tipWord =
      tempDom.tagName == 'DIV'
        ? tempDom.querySelector('input').getAttribute('placeholder')
        : tempDom.getAttribute('placeholder')
    tipWord = '请' + tipWord
    val =
      tempDom.className.indexOf('el-input') != -1
        ? tempDom.querySelector('input').value
        : tempDom.value
  }
  if (!val) {
    return {
  ...
  if (validObj.type.indexOf('phone') != -1) {
    //校验手机
  ...
  if (validObj.type.indexOf('vfCode') != -1) {
    //校验验证码
    if (reg.vfCode.test(val))
      return {
        pass: true
      }
    else
...
const handle = (validResult, isHandle) => {
  if (isHandle) {
    //校验不通过时添加class
    let d =
      validResult.dom.tagName == 'DIV'
        ? validResult.dom.querySelector('input')
        : validResult.dom
